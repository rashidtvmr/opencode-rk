# install-oc2.ps1: install the oc2 release archive (windows-x64).
...
# Upgrade preserves history; uninstall removes only oc2.
param(
  [string]$Archive = "",
  [string]$Checksum = "",
  [string]$InstallDir = (Join-Path $HOME ".local\bin"),
  [string]$Version = $env:OC2_VERSION,
  [switch]$Uninstall,
  [switch]$Help
)
...
function Usage {
  Write-Host "usage: install-oc2.ps1 [-Archive FILE] [-Checksum SHA256] [-InstallDir DIR] [-Uninstall]"
}
...
if (Test-Path -LiteralPath $legacy) {
  Write-Host "refusing: $legacy exists; oc2 installs side-by-side only"
  exit 73
}

$stage = Join-Path ([System.IO.Path]::GetTempPath()) ("oc2-" + [System.Guid]::NewGuid())
