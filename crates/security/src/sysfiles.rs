...
impl SystemFilePolicy {
    ...
    pub fn check_read(&self, path: impl AsRef<Path>) -> Result<(), SystemFileDenied> {
        ...
        if self.trusted && n.starts_with("/etc/") {
            return Ok(());
        }
        if READ_ALLOWED.contains(&n.as_str()) {
            return Ok(());
        }
...
impl Default for SystemFilePolicy {
    fn default() -> Self {
        Self::new(false)
    }
}
fn denied(path: &str, reason: &str) -> SystemFileDenied {
    SystemFileDenied {
        path: path.to_owned(),
...
fn is_sys(n: &str) -> bool {
    n.starts_with("/proc/") || n == "/proc" || n.starts_with("/sys")
}
...
mod tests {
    use super::*;
    fn untrusted() -> SystemFilePolicy {
        SystemFilePolicy::new(false)
    }
    #[test]
    fn read_etc_hosts_allowed() {
        assert!(untrusted().check_read("/etc/hosts").is_ok());
    }
    #[test]
    fn write_etc_hosts_denied() {
        assert!(untrusted().check_write("/etc/hosts").is_err());
    }
    #[test]
    fn trusted_read_more() {
        assert!(SystemFilePolicy::new(true)
            .check_read("/etc/passwd")
            .is_ok());
    }
    #[test]
    fn trusted_write_still_denied() {
        let p = SystemFilePolicy::new(true);
        assert!(p.check_write("/etc/anything").is_err());
        assert!(p.check_write("/usr/bin/x").is_err());
    }
    #[test]
    fn non_system_allowed() {
