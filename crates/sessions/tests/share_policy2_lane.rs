//! SHARE-005 frozen tests T01..T05 (fallback lane).
//! Deterministic share-sync transport policy: at-most-once per seq,
//! typed retry/abort, explicit delete confirmation. `#[path]` include:
//! `share_policy2_lane.rs` is owned by this lane and is NOT wired into
//! `lib.rs` (integrator assembles shared files).
#[path = "../src/share_policy2_lane.rs"]
