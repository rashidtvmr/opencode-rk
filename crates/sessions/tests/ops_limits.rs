use opencode_rk_sessions::ops_limits::{OpLimits, OpLimitsError, check_limits, tasks_within};

#[test]
fn oplim_t01_ok() {
    let l = OpLimits { max_tasks: 4, max_mb: 64 };
    assert!(check_limits(&l).is_ok());
}

#[test]
fn oplim_t02_zero_tasks() {
    let l = OpLimits { max_tasks: 0, max_mb: 64 };
    assert!(matches!(check_limits(&l), Err(OpLimitsError::ZeroTasks)));
}

#[test]
fn oplim_t03_zero_mb() {
    let l = OpLimits { max_tasks: 4, max_mb: 0 };
