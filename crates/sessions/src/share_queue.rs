//! SHARE-002 bounded local share-event coalescing queue.
//!
...
pub struct CoalescingQueue {
    ...
    entries: BTreeMap<FullKey, ShareEvent>,
...
impl fmt::Debug for CoalescingQueue {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        f.debug_struct("CoalescingQueue")
...
impl CoalescingQueue {
    pub fn new(caps: QueueCaps, accepts: impl Fn(&str) -> bool + 'static) -> Self {
        Self {
            caps,
    ...
    fn full_key(event: &ShareEvent) -> FullKey {
        (
            event.session.clone(),
    ...
    pub fn push(&mut self, event: ShareEvent) -> Result<(), QueueError> {
        if self.finalized {
    ...
    pub fn drain(&mut self) -> Vec<ShareEvent> {
        let out: Vec<ShareEvent> = self.entries.values().cloned().collect();
        self.entries.clear();
        self.order.clear();
    ...
    pub fn requeue(&mut self, batch: Vec<ShareEvent>) -> Result<(), QueueError> {
        if self.finalized {
            return Err(QueueError::Finalized);
        }
        for event in batch {
