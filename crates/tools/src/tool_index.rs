...
fn is_readable_dir(root: &Path) -> bool {
    std::fs::metadata(root).map(|m| m.is_dir()).unwrap_or(false) && std::fs::read_dir(root).is_ok()
}
...
fn sorted_dir_entries(dir: &Path) -> Result<Vec<PathBuf>, ()> {
    ...
    out.sort_by(|a, b| (file_label(a), a).cmp(&(file_label(b), b)));
    Ok(out)
}
...
fn parse_symbol_decl(line: &str) -> Option<(SymbolKind, String)> {
    ...
    .find_map(|(kw, kind)| {
        rest.strip_prefix(kw)
            .filter(|after| {
                after.is_empty() || after.chars().next().is_none_or(|c| !is_ident_char(c))
            })
...
fn brace_end(lines: &[&str], decl_idx: usize) -> usize {
    ...
    if seen_open { lines.len() } else { decl_idx + 1 }
}
