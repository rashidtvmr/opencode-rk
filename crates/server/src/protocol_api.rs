...
//! Handlers are thin: decode at the caller (WEB-001), call exactly one domain
//! service method, translate at the caller (WEB-002). No I/O, no wall-clock,
//! no secrets in the table. Static slice, O(routes). Std only.
...
/// Coverage failure. Carries at most the offending (method, path) key,
...
/// Check one served list against the authoritative table: table cap holds,
/// no duplicates in either the table or the served list, and every served
/// (method, path) has a Protocol entry. Pure, synchronous, no I/O.
...
/// routes are covered by Protocol.
...
/// Thin-handler dispatch for `POST /control/move_session`: calls exactly
/// one domain service method. Input decoding (WEB-001) and error
/// translation (WEB-002) happen at the caller. Performs no I/O itself.
pub fn dispatch_move_session(service: &impl MoveSessionService) {
    service.move_session();
}

/// Calls the thin handler made for one request. Thin means exactly one.
#[must_use]
pub fn domain_calls_per_request(spy: &DomainCallSpy) -> usize {
    spy.calls()
}

/// Substrings proving a handler module reaches mutating I/O directly
/// (git / filesystem / network) instead of going through the domain service.
...
#[must_use]
pub fn no_direct_git_fs_net_imports(handlers: &[(&str, &str)]) -> Vec<String> {
