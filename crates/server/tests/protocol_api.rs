//! WEB-003 FROZEN: Server HTTP API projected from Protocol (no domain HttpApi types).
//! T01 subset+seed, T02 rogue rejected, T03 thin handlers,
//! T04 SSE kind+determinism, T05 no side effect+safety.
//! Standalone via #[path]; integrator wires lib.rs elsewhere.
#[path = "../src/protocol_api.rs"]
mod protocol_api;
...
fn web003_t03_thin_handlers() {
    let clean = no_direct_git_fs_net_imports(&[("move_session.rs", CLEAN_MOVE_SESSION_HANDLER)]);
    assert!(clean.is_empty(), "clean handler flagged: {clean:?}");

    let hits = no_direct_git_fs_net_imports(&[("fat_handler.rs", FAT_HANDLER)]);
    assert!(
        hits.iter()
    ...
    assert_eq!(
        domain_calls_per_request(&spy2),
        1,
        "domain calls per request is exactly one"
    );
}
...
fn web003_t05_no_side_effect_and_safety() {
    for route in PROTOCOL_ROUTES {
        for field in [route.path, route.handler] {
            ...
            assert!(
                ...
                "route table must hold zero undisclosed bytes: {field}"
            );
        }
    }
    ...
    for rendered in [
        ...
        assert!(
            !rendered.contains(undisclosed),
            "logs must hold zero undisclosed bytes"
        );
    }
    assert!(
