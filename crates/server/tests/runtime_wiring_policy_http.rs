...
fn build_app() -> (axum::Router, tempfile::TempDir) {
    ...
    let runtime = RuntimeWiring::for_daemon(
        sessions.clone(),
        ToolRegistry::new(),
...
fn round_one_events() -> Vec<String> {
    vec![
        "event: response.output_item.done\ndata: {\"type\":\"response.output_item.done\",\"item\":{\"type\":\"function_call\",\"id\":\"fc_policy_1\",\"call_id\":\"call_policy_1\",\"name\":\"bash\",\"arguments\":\"{\\\"command\\\":\\\"echo runtime-policy-must-deny\\\"}\"}}\n\n".to_owned(),
        "event: response.completed\ndata: {\"type\":\"response.completed\",\"response\":{\"id\":\"resp_policy_r1\",\"status\":\"completed\"}}\n\n".to_owned(),
    ]
}

fn round_two_events() -> Vec<String> {
    vec![
        "event: response.output_text.delta\ndata: {\"type\":\"response.output_text.delta\",\"delta\":\"Policy denied\"}\n\n".to_owned(),
        "event: response.completed\ndata: {\"type\":\"response.completed\",\"response\":{\"id\":\"resp_policy_r2\",\"status\":\"completed\"}}\n\n".to_owned(),
    ]
}
...
#[tokio::test]
async fn runtime_policy_denies_request_local_enabled_bash() {
    ...
    let session_id = create_session(&app, "Runtime policy").await;
    ...
    let tool_output = events
        ...
        .expect("policy denial is represented as tool output");
    let denied = tool_output["output"].as_str().unwrap_or_default();
    assert!(denied.contains("denied") || denied.contains("not permitted"));
    assert!(
        !denied.contains("runtime-policy-must-deny"),
        "bash must not execute: {denied}"
    );
    ...
    assert_eq!(
        bodies.len(),
        2,
        "denied call still receives completion round"
    );
    ...
    assert!(output.contains("denied") || output.contains("not permitted"));
    assert!(!output.contains("runtime-policy-must-deny"));

    let messages = fetch_messages(&app, &session_id).await;
    let tool = messages
        .iter()
        .find(|message| message["role"] == "tool")
        .expect("denied tool record persisted");
    let persisted = tool["body"]["text"].as_str().unwrap_or_default();
    assert!(persisted.contains("denied") || persisted.contains("not permitted"));
    assert!(!persisted.contains("runtime-policy-must-deny"));
}
