//! WEB-002 RED: domain-error to HTTP translation at the route boundary.
...
#[path = "../src/error_translate.rs"]
mod error_translate;

use error_translate::{
    DomainError, MAX_ERROR_MESSAGE_BYTES, all_codes, move_session, translate, wire_body,
};
...
fn web002_t04_all_variants_covered_exactly_once() {
    ...
    let src = include_str!("../src/error_translate.rs");
    assert!(
        !src.contains("_ =>"),
...
// WEB-002-T05: domain stays HTTP-free + failing path is side-effect free.
#[test]
fn web002_t05_domain_http_free_and_no_side_effects() {
    let src = include_str!("../src/error_translate.rs");
...
// Domain service signature stays HTTP-free: pure decision, no transport types.
#[test]
fn web002_t05_domain_move_session_returns_typed_errors() {
