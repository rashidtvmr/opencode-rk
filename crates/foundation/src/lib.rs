...
use thiserror::Error;
use tokio::{sync::{mpsc, Mutex, OnceCell, Semaphore}, task::{JoinHandle, JoinSet}, time};
...
#[derive(Debug, Error, Eq, PartialEq)] pub enum RuntimeError { #[error("runtime limit {0} must be non-zero")] InvalidLimit(&'static str), #[error("task scope is shutting down")] ShuttingDown, #[error("owned task limit reached")] TaskLimitReached, #[error("task shutdown exceeded grace period")] ShutdownTimedOut, #[error("owned task panicked or was cancelled: {0}")] Join(String), #[error("byte budget exceeded: requested {requested}, available {available}")] BudgetExceeded { requested: usize, available: usize }, #[error("service initialization failed: {0}")] ServiceInit(String), #[error("service concurrency limit reached")] ServiceLimitReached }
...
#[derive(Clone, Debug)]
pub struct ByteBudget { limit: usize, used: Arc<AtomicUsize> }
impl ByteBudget {
    pub fn new(limit: usize) -> Result<Self, RuntimeError> { if limit == 0 { return Err(RuntimeError::InvalidLimit("byte_budget")); } Ok(Self { limit, used: Arc::new(AtomicUsize::new(0)) }) }
    pub fn acquire(&self, bytes: usize) -> Result<(), RuntimeError> {
        ...
        loop {
            ...
            if current.checked_add(bytes).map_or(true, |next| next > self.limit) {
                return Err(RuntimeError::BudgetExceeded { requested: bytes, available });
            }
            match self.used.compare_exchange_weak(current, current + bytes, Ordering::SeqCst, Ordering::SeqCst) {
                Ok(_) => return Ok(()),
    ...
    pub fn reserve(&self, bytes: usize) -> Result<ByteBudgetReservation, RuntimeError> { self.acquire(bytes)?; Ok(ByteBudgetReservation { budget: self.clone(), bytes }) }
    #[must_use] pub fn used(&self) -> usize { self.used.load(Ordering::SeqCst) }
    #[must_use] pub fn limit(&self) -> usize { self.limit }
    #[must_use] pub fn available(&self) -> usize { self.limit.saturating_sub(self.used.load(Ordering::SeqCst)) }
    pub fn try_consume(&self, bytes: usize) -> Result<(), RuntimeError> { self.acquire(bytes) }
    #[must_use] pub fn remaining(&self) -> usize { self.available() }
    pub fn reset(&self) { self.release(self.used()) }
}

#[must_use = "reservations auto-release on drop unless leaked"]
#[derive(Debug)]
pub struct ByteBudgetReservation { budget: ByteBudget, bytes: usize }
impl ByteBudgetReservation {
    pub fn leak(mut self) -> usize { let b = self.bytes; self.bytes = 0; b }
    #[must_use] pub fn bytes(&self) -> usize { self.bytes }
}
impl Drop for ByteBudgetReservation { fn drop(&mut self) { if self.bytes > 0 { self.budget.release(self.bytes); } } }

type AsyncInit<T> = Box<dyn Fn() -> Pin<Box<dyn Future<Output = Result<T, RuntimeError>> + Send>> + Send + Sync>;

pub struct LazyService<T> { cell: OnceCell<Arc<T>>, init: Mutex<Option<AsyncInit<T>>>, permits: Option<Arc<Semaphore>> }
impl<T: fmt::Debug> fmt::Debug for LazyService<T> {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        f.debug_struct("LazyService").field("initialized", &self.cell.initialized()).field("service", &self.cell.get()).finish()
    }
}
pub struct LazyServicePermit<T> { service: Arc<T>, _permit: Option<tokio::sync::OwnedSemaphorePermit> }
impl<T: fmt::Debug> fmt::Debug for LazyServicePermit<T> {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result { f.debug_struct("LazyServicePermit").field("service", &self.service).finish() }
}
impl<T> Deref for LazyServicePermit<T> { type Target = T; fn deref(&self) -> &Self::Target { &self.service } }

impl<T: Send + Sync + 'static> LazyService<T> {
    pub fn new<F, Fut>(init: F) -> Self where F: Fn() -> Fut + Send + Sync + 'static, Fut: Future<Output = Result<T, RuntimeError>> + Send + 'static {
        Self { cell: OnceCell::new(), init: Mutex::new(Some(Box::new(move || Box::pin(init())))), permits: None }
    }
    pub fn bounded<F, Fut>(max_concurrency: usize, init: F) -> Result<Self, RuntimeError> where F: Fn() -> Fut + Send + Sync + 'static, Fut: Future<Output = Result<T, RuntimeError>> + Send + 'static {
        if max_concurrency == 0 { return Err(RuntimeError::InvalidLimit("lazy_service_concurrency")); }
        Ok(Self { cell: OnceCell::new(), init: Mutex::new(Some(Box::new(move || Box::pin(init())))), permits: Some(Arc::new(Semaphore::new(max_concurrency))) })
    }
    pub async fn get(&self) -> Result<Arc<T>, RuntimeError> {
        self.cell.get_or_try_init(|| async {
            let init_fn = {
                let guard = self.init.lock().await;
                guard.as_ref().map(|f| f()).ok_or_else(|| RuntimeError::ServiceInit("initializer unavailable".to_string()))?
            };
            let service = init_fn.await?;
            Ok(Arc::new(service))
        }).await.cloned()
    }
    pub async fn acquire(&self) -> Result<LazyServicePermit<T>, RuntimeError> { let service = self.get().await?;
        let permit = match &self.permits {
            Some(sem) => Some(Arc::clone(sem).acquire_owned().await.map_err(|_| RuntimeError::ShuttingDown)?),
            None => None,
        };
        Ok(LazyServicePermit { service, _permit: permit })
    }
    pub fn try_acquire(&self) -> Result<LazyServicePermit<T>, RuntimeError> {
        let service = self.cell.get().cloned().ok_or_else(|| RuntimeError::ServiceInit("service not initialized".to_string()))?;
        let permit = match &self.permits {
            Some(sem) => Some(Arc::clone(sem).try_acquire_owned().map_err(|_| RuntimeError::ServiceLimitReached)?),
            None => None,
        };
        Ok(LazyServicePermit { service, _permit: permit })
    }
    #[must_use] pub fn is_initialized(&self) -> bool { self.cell.initialized() }
}

#[derive(Debug)]
pub struct HeartbeatGuard { token: CancellationToken, handle: Option<JoinHandle<()>>, ticks: Arc<AtomicU64> }
impl HeartbeatGuard {
    pub fn spawn<F, Fut>(interval: Duration, mut action: F) -> Result<Self, RuntimeError>
    where F: FnMut(u64) -> Fut + Send + 'static, Fut: Future<Output = ()> + Send + 'static {
        if interval.is_zero() { return Err(RuntimeError::InvalidLimit("heartbeat_interval")); }
        ...
        Ok(Self { token, handle: Some(handle), ticks })
    }
    pub fn spawn_with_token<F, Fut>(parent: &CancellationToken, interval: Duration, mut action: F) -> Result<Self, RuntimeError>
    where F: FnMut(u64) -> Fut + Send + 'static, Fut: Future<Output = ()> + Send + 'static {
        if interval.is_zero() { return Err(RuntimeError::InvalidLimit("heartbeat_interval")); }
    ...
    #[must_use] pub fn ticks(&self) -> u64 { self.ticks.load(Ordering::SeqCst) }
    #[must_use] pub fn cancellation_token(&self) -> CancellationToken { self.token.child_token() }
    #[must_use] pub fn is_cancelled(&self) -> bool { self.token.is_cancelled() }
...
#[cfg(test)] mod tests {
    ...
    #[test] fn byte_budget_enforces_limit() {
        ...
        assert_eq!(budget.acquire(50), Err(RuntimeError::BudgetExceeded { requested: 50, available: 40 }));
        budget.release(30);
    ...
    #[test] fn byte_budget_reservation_auto_releases() {
        let budget = ByteBudget::new(100).unwrap();
        {
    ...
    #[test] fn byte_budget_rejects_zero_limit() {
        assert_eq!(ByteBudget::new(0).unwrap_err(), RuntimeError::InvalidLimit("byte_budget"));
    }

    #[tokio::test] async fn lazy_service_constructs_on_first_use() {
    ...
    #[tokio::test] async fn lazy_service_bounded_concurrency() {
        let service = LazyService::bounded(1, || async { Ok(42) }).unwrap();
        let permit1 = service.acquire().await.unwrap();
        assert_eq!(*permit1, 42);

        assert_eq!(service.try_acquire().err(), Some(RuntimeError::ServiceLimitReached));
        drop(permit1);
    ...
    #[tokio::test] async fn heartbeat_guard_ticks_and_cancels_on_drop() {
        let ticks = Arc::new(AtomicU64::new(0));
        let ticks_clone = Arc::clone(&ticks);
    ...
    #[test] fn heartbeat_guard_rejects_zero_interval() {
        assert_eq!(HeartbeatGuard::spawn(Duration::ZERO, |_| async {}).unwrap_err(), RuntimeError::InvalidLimit("heartbeat_interval"));
    }

    #[test] fn byte_budget_aliases_try_consume_remaining_reset(){let b=ByteBudget::new(10).unwrap(); b.try_consume(4).unwrap(); assert_eq!(b.remaining(),6); assert_eq!(b.try_consume(7),Err(RuntimeError::BudgetExceeded{requested:7,available:6})); b.reset(); assert_eq!(b.used(),0); assert_eq!(b.remaining(),10);}
}
