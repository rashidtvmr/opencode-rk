...
//! bounded connect over a caller-provided transport, reclaim on close.
//! Handles are opaque ids; raw tokens never cross this boundary. No
//! network, no threads, no persistence; validation performs zero I/O.
...
impl McpAttachment {
    ...
    /// Bounded connect/handshake over the caller-provided transport.
    ///
    ...
    pub fn connect(
        ...
        // Script ownership stays with the transport; read steps by index so
        // the fixture remains reusable and no allocation escapes on failure.
