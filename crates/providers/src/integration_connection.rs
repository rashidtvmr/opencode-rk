//! INT-002: caller-driven key/OAuth connection projection.
//!
//! Pure in-memory table over caller-supplied material. The `known` oracle is a
//! caller-supplied `&dyn Fn(&str, &str) -> Option<MethodKind>`; borrowed code
//! bytes are consumed for the verdict only and never retained. No network,
//! no provider callback/refresh, no persistence, no events, no threads.

use std::fmt;

use thiserror::Error;

/// Maximum retained connections per table (frozen by INT-002-T03).
pub const MAX_CONNECTIONS: usize = 64;
/// Maximum accepted authorization-code length in chars (frozen by INT-002-T03).
pub const MAX_CODE_LEN: usize = 512;
/// Name/method length caps, same charset rules as INT-001 ids.
const MAX_NAME_LEN: usize = 64;

/// Method kind answered by the caller-supplied oracle.
...
/// Connect request. `code` is plain `Option<String>` for caller ergonomics;
/// the implementation borrows it for the verdict and never clones it into
/// retained state.
...
impl fmt::Debug for ConnectInput {
...
impl ConnectionTable {
    ...
    /// Project a connect: validate input, ask the caller oracle, record
