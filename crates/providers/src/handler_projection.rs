//! Typed-integration-handler projection. Pure, no IO.
//!
//! Projects a caller-supplied typed integration handler outcome into its
//! normalized HTTP-facing shape: secret-free projection, `NoContent`
//! mutation ack, or typed `InvalidRequest` / auth / code-required error.
//! Auth failures normalize to one shape with zero provider detail, and
//! code-mode completion without a code stays pending as code-required
//! rather than failing as auth. No credentials, env, storage, network,
//! provider callbacks, or events are touched.
