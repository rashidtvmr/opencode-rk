//! INT-010 fallback lane: pure share-descriptor validation + last-write-wins merge.
//!
//! Distinct fallback file from `share_descriptor.rs` (same slice, no
//! collision). Caller supplies descriptors; `key_ref` is an opaque reference
//! (never secret bytes). Synchronous in-memory view, no network, no
//! persistence, no env reads, no threads, no clock. Hosted partitions are
//! refused by type via [`SyncOp::ProbeHosted`].
...
/// Caller-supplied share descriptor. `digest` is a caller-computed 64-hex
/// payload digest; `seq` is caller-supplied and monotonic per `id`.
...
fn view_of(desc: &ShareDesc) -> ShareView {
    ShareView {
        id: desc.id.clone(),
...
impl ShareSync {
    /// Empty view. Allocates only the empty map.
    #[must_use]
    pub fn new() -> Self {
        Self::default()
    }
    ...
    /// Latest view for `id`, if present.
    #[must_use]
    pub fn get(&self, id: &str) -> Option<ShareView> {
        self.shares.get(id).cloned()
    }

    /// All views sorted by `id` ascending (BTreeMap order).
    #[must_use]
