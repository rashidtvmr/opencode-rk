use opencode_rk_providers::recording::{
    summarize_calls, RecordedCall, RecordingError, MAX_RECORDED_CALLS,
};
...
fn recording_t05_overflow_rejected() {
    let calls: Vec<RecordedCall> = (0..MAX_RECORDED_CALLS + 1)
        .map(|_| call("a", 1, 1, false))
        .collect();
    let err = summarize_calls(&calls).expect_err("overflow");
    assert!(
        matches!(err, RecordingError::TooManyCalls { max, actual } if max == MAX_RECORDED_CALLS && actual == MAX_RECORDED_CALLS + 1)
    );
}
