...
//! wire `crates/providers/src/int_handler_lane.rs` into `lib.rs` without
//! this lane touching shared files. Distinct lane file from
//! `handler_projection.rs` (same slice fallback, no collision).

#[path = "../src/int_handler_lane.rs"]
mod int_handler_lane;

use int_handler_lane::{
...
fn int_007_t01_list_get_keyack_happy_path() {
    ...
    match int_handler_lane::project(&HandlerOp::List, None, &items) {
    ...
    let first = format!(
        "{:?}",
        int_handler_lane::project(&HandlerOp::List, None, &items)
    );
    let second = format!(
        "{:?}",
        int_handler_lane::project(&HandlerOp::List, None, &items)
    ...
    match int_handler_lane::project(&HandlerOp::Get, Some(&a), &items) {
    ...
    match int_handler_lane::project(&HandlerOp::KeyAck, Some(&a), &items) {
        HandlerResponse::NoContent => {}
        other => panic!("expected NoContent, got {other:?}"),
    }
    match int_handler_lane::project(&HandlerOp::OAuthStart, Some(&b), &items) {
        HandlerResponse::NoContent => {}
        other => panic!("expected NoContent, got {other:?}"),
    }
}
...
fn int_007_t02_code_required_vs_auth_failed() {
    ...
    let no_code = int_handler_lane::project(
    ...
    let flagged_no_code = int_handler_lane::project(
        &HandlerOp::OAuthComplete {
            code: None,
            auth_failed: true,
        },
    ...
    let auth_failed = int_handler_lane::project(
        &HandlerOp::OAuthComplete {
...
fn int_007_t03_validation_and_missing() {
    ...
    for id in &bad_ids {
        ...
        match int_handler_lane::project(&HandlerOp::KeyAck, Some(&m), &items) {
            HandlerResponse::Err(HandlerError::InvalidRequest) => {}
            other => panic!("expected InvalidRequest for id {id:?}, got {other:?}"),
        }
        match int_handler_lane::project(&HandlerOp::OAuthStart, Some(&m), &items) {
            HandlerResponse::Err(HandlerError::InvalidRequest) => {}
            other => panic!("expected InvalidRequest for id {id:?}, got {other:?}"),
        }
    }
    ...
    match int_handler_lane::project(&HandlerOp::Get, None, &items) {
        HandlerResponse::Err(HandlerError::NotFound) => {}
        other => panic!("expected NotFound for Get(absent), got {other:?}"),
    }
    match int_handler_lane::project(&HandlerOp::AttemptPoll, None, &items) {
        HandlerResponse::Err(HandlerError::NotFound) => {}
...
fn int_007_t04_normalization_and_truncation() {
    ...
    let r1 = int_handler_lane::project(
        &HandlerOp::OAuthComplete {
    ...
    let r2 = int_handler_lane::project(
        &HandlerOp::OAuthComplete {
    ...
    let mut big: Vec<NonSecretMeta> = (0..200)
        ...
        .collect();
    big.reverse();
    match int_handler_lane::project(&HandlerOp::List, None, &big) {
        HandlerResponse::List { items, truncated } => {
...
fn int_007_t05_no_authority_safety() {
    ...
    for op in &ops {
        for meta in [None, Some(&m)] {
            let resp = int_handler_lane::project(op, meta, &items);
            apply(&resp, &mut mutated);
            log_capture.push_str(&format!("{resp:?} "));
        }
    }
    let bad = key("", "");
    let resp = int_handler_lane::project(&HandlerOp::KeyAck, Some(&bad), &items);
    log_capture.push_str(&format!("{resp:?}"));
