...
## Durable exhaustion normalization and drift guard

The next source/docs/validation-only pass starts from clean `main` at `efad109` and rechecks the entire non-controller-accepted set instead of relying on the preceding prose totals. The live partition remains exact and disjoint: 219 total stories = 133 controller-accepted + 21 Ralph-stale local `IMPLEMENTED` + 9 explicit blockers + 22 dependency-constrained UI/client + 34 unresolved/decomposition-blocked. All 86 nonaccepted IDs occur exactly once, with no missing, duplicate, accepted-as-residual, or extra story.

The audit also found a real durable-state defect: `FEATURES.md` describes itself as generated from `ralph.json`, but its story-status mirrors were stale for 163 rows (all 133 controller-accepted stories plus all 30 live `in-progress` stories were still rendered as `not-started`). This pass synchronizes only those generated status tokens from Ralph; it does not alter user-story text, requirements, test obligations, task semantics, or any Ralph/controller field. The new validator now rejects future `FEATURES.md` status drift.

`sources/backlog-exhaustion.json` is the canonical local exhaustion-accounting artifact for the current native lane. It stores only the 86 non-controller-accepted rows; controller-accepted membership and counts are always derived from live `ralph.json`, not duplicated as an editable accepted-ID list. Each row records its controller status, category, requirements, typed blocker/reopen policy, mapped DISC surface IDs, the exact reviewed evidence IDs inherited from those surfaces, local evidence paths, task/worklog presence and task status, and known implementation commit(s) for the 21 stale-local rows. Evidence IDs resolve through the existing DISC catalogs to exact locked repository commits, paths, and Git blob SHAs; `tools/validate_backlog_exhaustion.py` independently verifies those pins as well as the canonical DISC reconciliation validator.

Documentation is intentionally normalized without fabricating contracts. All 21 stale-local implementations already have a real task card and worklog and remain candidate/local until external verifier/controller acceptance. The nine explicit blockers and all 34 unresolved/decomposition rows still lack a source-grounded one-to-one contract, so placeholder task cards/worklogs are not created merely to make file counts look complete. Among the 22 dependency-constrained rows, UI-014..UI-018 retain their existing source-grounded target cards but no implementation worklog; the other client/UI rows remain represented by the reviewed client surfaces and exhaustion ledger until an approved native architecture/dependency boundary exists. A row leaves blocker/unresolved classification only when materially new evidence establishes unique ownership; only then should a real task card define the contract and a worklog begin with its independent RED lane.

The new guard is integrated into `/usr/bin/python3 tools/validate_plan.py`. Bootstrap mutation coverage rejects a missing row, duplicate row, controller-accepted row in the residual ledger, movement of a pinned explicit blocker or dependency-constrained ID into another bucket, silent surface/evidence projection drift, and DISC-003 reconciliation/manifest/source-map promotion toward accepted/release evidence. The validator additionally requires every stale-local row's task card to remain `IMPLEMENTED`, its worklog to exist, its recorded implementation commit(s) to resolve in git history, and detects a newly `IMPLEMENTED` nonaccepted card that was not added to stale-local accounting.

No materially new pinned source evidence appeared in this pass. The nine explicit do-not-reopen blockers remain unresolved on their recorded grounds, and none of the 34 unresolved/decomposition rows acquires a new exact one-to-one native product owner. The product lane therefore remains exhausted under the current constraints, but the state is now mechanically guarded rather than prose-only. DISC-003 remains **IN PROGRESS / NOT ACCEPTED** with `in-progress-not-release-evidence`; this normalization does not claim project completion, release evidence, or verifier/controller acceptance.

Validation for the durable exhaustion-normalization pass:

- `/usr/bin/python3 -m unittest discover -s tests/bootstrap -p 'test_*.py' -v` => 149/149 passed. The pass intentionally repaired two stale bootstrap fixtures exposed by the wider consistency sweep: `test_auto_controller.py` now tests readiness using an isolated all-not-started fixture while separately checking the live 219-story status totals, and `test_freeze_sources.py` supplies the required SPDX field in its otherwise-valid source-lock fixture.
- `/usr/bin/python3 -m unittest tests.bootstrap.test_backlog_exhaustion -v` => 8/8 passed, including missing/duplicate/accepted-residual/category/evidence drift and DISC-003 non-acceptance mutation coverage.
- `/usr/bin/python3 tools/validate_backlog_exhaustion.py` => `OK  stories=219 accepted=133 stale=21 blockers=9 dependency=22 unresolved=34`.
- `/usr/bin/python3 tools/reconcile_surfaces.py` => passed with exactly 32 reviewed `partial`, 0 queued, implementation statuses 24 `partial` / 8 `reference-implemented`, 179 evidence references, 79 unresolved findings, zero errors.
- `/usr/bin/python3 tools/validate_plan.py` => `validate_plan: OK  stories=219 requirements=38 obligations=1095 deps_synthesized=True`; backlog exhaustion is now a required plan-validation input.
- `/usr/bin/python3 tools/lane_gate.py --run` => all 12 storage-format-2 module/test lanes PASS. This pass does not edit storage; the gate is retained as an integrated regression check because it is part of the repository's configured controller verification commands.
- `FEATURES.md` was compared to its pre-pass revision after replacing only status tokens with a sentinel; the normalized texts are byte-equivalent, proving this pass changed no feature prose while synchronizing the 163 stale status mirrors.
- All three frozen INT-008 test hashes remain unchanged: `3b1a5d5153c88862943ac0d5eacba67987fb94e0a7f25649fac32715617f3f2c`, `62ad83f617d42bcb2778f678a5e4b690560bb4b3e3f1e0cc38130483f7fd0b30`, and `a9255bdb22014459596fc667a833ef0a5a283dad7b16922003bbe9e0b07328ca`.
- `jq empty` passes for `sources/backlog-exhaustion.json` plus the four canonical DISC JSON artifacts, `git diff --check` passes, and `git diff --name-only -- ralph.json` is empty. `ralph.json` remains unchanged.

## Pull-request and controller enforcement integration

The follow-up pass from clean `main` at `41586c2` inspected the real validation entrypoints rather than assuming that `validate_plan.py` alone represented CI. The planning job in `.github/workflows/ci.yml` ran `validate_plan.py` plus bootstrap discovery; the leased controller and `tools/auto_drive.py` also treated `validate_plan.py` as their repository-wide verification command. Because `validate_plan.py` includes backlog exhaustion this already guarded the 219-story partition indirectly, but none of those entrypoints executed canonical DISC-003 reconciliation or proved that the checked-in reconciliation manifest still matched its five hash-bound inputs.

`tools/reconcile_surfaces.py` therefore gains a read-only `--check-manifest` mode. It builds the canonical manifest in memory, compares the full checked-in object, and fails closed on status/count/hash/warning drift or a missing manifest without rewriting any file. The existing default remains the explicit manifest-writing path for reconciliation work. `tests/bootstrap/test_ci_enforcement.py` proves the checked-in manifest matches current reconciliation, behavior rules, evidence catalogs, and Ralph, and includes a stale-count mutation so a cosmetically plausible but outdated manifest cannot pass.

`tools/validate_repository.py` is now the single normal repository guard. It deliberately runs three independent checks in order: `tools/validate_backlog_exhaustion.py`, `tools/reconcile_surfaces.py --check-manifest`, and `tools/validate_plan.py`. Exhaustion remains duplicated inside `validate_plan.py` as defense in depth rather than being removed from the older structural validator. Pull-request CI invokes the canonical guard directly and then runs the whole bootstrap suite. Meta-tests pin both the `pull_request` trigger and those exact CI commands, pin the canonical three-check command manifest, and verify that the leased controller plus `auto_drive` route verification through the same repository guard.

The canonical validator also checks the planning workflow contract itself: the pull-request trigger, exact repository-validation/bootstrap command lines, stable `planning` job, and absence of conditional/`continue-on-error` bypasses. This catches accidental workflow weakening as long as the repository's planning status check still executes. A repository-local mutable workflow cannot protect itself against a deliberate same-change deletion of the entire required job; that final tamper boundary remains the external branch/ruleset policy and is not claimed as source-level enforcement.

The controller path is likewise fail-closed now. `tools/ralph_loop.py` treats canonical repository validation plus `lane_gate.py --run` as `MANDATORY_VERIFICATION_COMMANDS`; operator settings may append verification commands but cannot replace those two. Bootstrap coverage loads a synthetic settings file that attempts to replace verification with `/bin/true` and proves both mandatory commands remain first. `auto_drive` directly invokes the canonical repository validator as its post-worker verification path.

Focused exhaustion mutations now also reject stale summary counts and disappearing stale-local receipts. A mutated INT-008 ledger row cannot drop its worklog or implementation commits without projection drift, and a simulated missing git history makes every stale-local implementation receipt fail explicitly. These additions complement the existing missing/double classification, accepted-as-residual, blocker/dependency category, surface/evidence, and DISC-003 acceptance-state mutation checks.

Documentation mirrors were normalized without product semantics: the README's old preparation-time `193 / 965 / 35` counts are updated to the live `219 / 1095 / 38`, the bootstrap command points at the canonical repository validator, and its obsolete claim that the Rust workspace is only an unimplemented scaffold is replaced with the current partial/externally-accepted state. No task card, verifier receipt, Ralph acceptance field, behavior-surface mapping, pinned evidence record, or product crate is changed by this enforcement integration. DISC-003 remains **IN PROGRESS / NOT ACCEPTED** and the manifest/reconciliation status remains `in-progress-not-release-evidence`.

Validation for the enforcement integration:

- `/usr/bin/python3 -m unittest discover -s tests/bootstrap -p 'test_*.py' -q` => 155/155 passed. The suite includes exact CI-command/meta enforcement, checked-in DISC manifest freshness, non-overridable controller verification, missing/double/stale-count/category classification mutations, stale-local receipt mutations, and DISC acceptance-state guards.
- `/usr/bin/python3 tools/validate_backlog_exhaustion.py` => `OK  stories=219 accepted=133 stale=21 blockers=9 dependency=22 unresolved=34`.
- `/usr/bin/python3 -m unittest tests.bootstrap.test_disc003_reconciliation -q` => 8/8 passed.
- `/usr/bin/python3 tools/reconcile_surfaces.py --check-manifest` => 32 `partial`, 0 queued, 24 `partial` + 8 `reference-implemented`, 179 evidence references, 79 unresolved findings, zero errors, `manifestMode: checked`.
- `/usr/bin/python3 tools/validate_plan.py` => `validate_plan: OK  stories=219 requirements=38 obligations=1095 deps_synthesized=True`.
- `/usr/bin/python3 tools/validate_repository.py` => `validate_repository: OK  exhaustion + DISC-003 + plan` after independently running all three required checks.
- `jq empty` passed for Ralph, the exhaustion/rules/evidence/reconciliation/manifest/upstream-lock/source-map JSON inputs, and controller settings. `/usr/bin/python3 -m py_compile` passed for all touched Python validation/controller/test files. `git diff --check` passed.
- `/usr/bin/python3 tools/lane_gate.py --run` => all 12 configured storage-format-2 module/test lanes PASS.
- `cargo fmt --all -- --check` remains nonzero because of broad pre-existing formatting drift across untouched Rust crates. This pass does not edit any Rust path (`git diff --name-only 41586c2 -- crates` is empty), so it does not rewrite product code or the frozen INT-008 suites merely to alter that unrelated baseline.
- Frozen INT-008 SHA-256 values remain `3b1a5d5153c88862943ac0d5eacba67987fb94e0a7f25649fac32715617f3f2c`, `62ad83f617d42bcb2778f678a5e4b690560bb4b3e3f1e0cc38130483f7fd0b30`, and `a9255bdb22014459596fc667a833ef0a5a283dad7b16922003bbe9e0b07328ca`. `ralph.json` has zero diff files.

## Post-enforcement residual evidence hardening

The immediate read-only re-evaluation on `17a312e` again found exactly 86 non-controller-accepted rows and the unchanged `21 / 9 / 22 / 34` residual partition. All 21 stale-local rows still have `IMPLEMENTED` task cards, worklogs, and resolvable implementation commits. The CI enforcement commit changed no Ralph row, `FEATURES.md`, behavior-surface rule, pinned evidence catalog, DISC reconciliation/manifest input, upstream lock, exhaustion-ledger row, or Rust product path relative to `41586c2`, so it introduced no new source contract or implementation owner.

The row-by-row evidence projection did identify one remaining guard gap worth closing. Exactly 65 of the 86 residual rows map to one or more reviewed DISC surfaces; when their mapped surfaces are aggregated, every one currently retains pinned `source`, `caller`, `test`, and `spec` evidence. The remaining 21 rows deliberately have no DISC surface (automation/release-assurance/current local-implementation/TUI-target cases) and are documented through local requirements/cards/worklogs or explicit no-owner status rather than fabricated upstream ownership. `tools/validate_backlog_exhaustion.py` now fails if any of those 65 surface-backed rows loses any of the four evidence classes, and bootstrap coverage removes `caller` evidence from `opencode.extensibility` to prove `EXT-001` can no longer remain exhaustion-classified silently under a weakened evidence trail.

This stronger evidence-class invariant does not promote any task. The source/caller/test/spec material is still family-level evidence, and the reviewed findings continue to state that residual EXT/INT/OPS/ROUTE/SHARE/WEB families require task-level decomposition or side-effect/authority separation. The nine explicit blockers remain frozen on their recorded ownership/authority conflicts, dependency-constrained UI/TUI remains excluded, and DISC-003 remains **IN PROGRESS / NOT ACCEPTED** with `in-progress-not-release-evidence`.

## Source-controlled repository protection policy

The next enforcement pass starts from clean `main` at `84e5765` and closes the remaining repository-local half of the workflow-tamper boundary. Before this pass the repository had no `CODEOWNERS` file and no checked-in branch/ruleset desired-state policy; the only self-protection was `tools/validate_repository.py` inspecting the mutable `planning` workflow while that workflow was already running. The canonical remote is `git@github.com:rashidtvmr/opencode-rk.git`, so `@rashidtvmr` is the only code-owner identity this pass can source-ground without inventing a team or collaborator.

`.github/CODEOWNERS` now explicitly assigns that owner to `.github/` itself plus the controller, canonical validators, exhaustion/DISC inputs, Ralph/FEATURES mirrors, relevant bootstrap tests, DISC task/progress state, and protection documentation. `.github/protection-policy.json` separately records the desired external `main` ruleset: require PRs, the stable `planning` status check with up-to-date branches, one approving/code-owner review, stale-approval dismissal, most-recent-push approval, conversation resolution, linear history, no force pushes/deletion, and no bypass. Its `platformState.verified` is intentionally and mechanically fixed to `false`; source cannot attest that those GitHub settings are active.

`tools/validate_protection_policy.py` fail-closes the source-controlled half. It pins the repository/owner identity, exact CODEOWNERS pattern order and owner, all 41 critical protected paths, the required `planning` context and desired external ruleset semantics, path existence/rename resistance, and the explicit non-attestation state. Bootstrap mutations alter the owner/pattern, rename `tools/validate_repository.py`, remove the required status check/enable bypass/disable code-owner review, and falsely set platform verification true; each must fail.

The canonical repository validator now runs protection policy first, then backlog exhaustion, read-only DISC manifest reconciliation, and plan validation. The `planning` workflow itself has an explicit `name: planning`, and its protected block is pinned to a signed full-SHA `actions/checkout` v4.4.0 revision with persisted credentials disabled, followed by canonical validation and complete bootstrap discovery. Its trigger/permission preamble is also pinned to an unfiltered `pull_request` trigger and `contents: read`. Conditional execution, `needs`, `continue-on-error`, job rename, command removal, or planning-block drift fail repository validation/tests. This is deliberately stronger against accidental edits, but a same-change deletion of the required workflow and all local guards still cannot be prevented by files inside that same Git tree; only an externally configured required status/ruleset and code-owner review can close that final boundary.

`docs/REPOSITORY_PROTECTION.md` documents both layers and the solo-owner caveat: a required independent/code-owner approval for a protected-path PR authored by `@rashidtvmr` may need a second real write-capable owner before the external ruleset can be activated without self-review deadlock. This pass does not invent that person or claim such an external setting already exists.

The policy review also exposed and removed one stale repository-documentation reference: `README.md` still named `feature-ledger.json`, but no such file exists in the live tree. The README now points at `sources/behavior-surface-rules.json`, which is the actual source-family-to-feature-scope candidate mapping. No replacement artifact was fabricated merely to satisfy the old prose.

Validation for the source-controlled protection pass:

- `/usr/bin/python3 -m unittest discover -s tests/bootstrap -p 'test_*.py' -q` => 160/160 passed before staging, including protection-path/owner/ruleset mutations and the existing CI/exhaustion/DISC/plan guards.
- `/usr/bin/python3 tools/validate_protection_policy.py` => `OK  owner=@rashidtvmr protected_paths=41 required_check=planning platform_verified=false`.
- `/usr/bin/python3 tools/validate_backlog_exhaustion.py` => `OK  stories=219 accepted=133 stale=21 blockers=9 dependency=22 unresolved=34`.
- `/usr/bin/python3 -m unittest tests.bootstrap.test_disc003_reconciliation -q` => 8/8 passed.
- `/usr/bin/python3 tools/reconcile_surfaces.py --check-manifest` and the canonical write-mode reconciliation both passed at exactly 32 reviewed `partial`, 0 queued, implementation statuses 24 `partial` / 8 `reference-implemented`, 179 evidence references, 79 unresolved findings, zero errors. Write mode produced no manifest diff.
- `/usr/bin/python3 tools/validate_plan.py` => `validate_plan: OK  stories=219 requirements=38 obligations=1095 deps_synthesized=True`.
- `/usr/bin/python3 tools/validate_repository.py` => `validate_repository: OK  protection + exhaustion + DISC-003 + plan`.
- `jq empty` passed across Ralph, the new protection policy, controller/resource/security config, user requirements, exhaustion/rules/evidence/DISC/upstream-lock/source-map JSON. `/usr/bin/python3 -m py_compile` passed for all touched validation/bootstrap Python files. `git diff --check` passed. `/usr/bin/python3 tools/lane_gate.py --run` kept all 12 configured storage-format-2 lanes PASS.
- Frozen INT-008 SHA-256 values remain `3b1a5d5153c88862943ac0d5eacba67987fb94e0a7f25649fac32715617f3f2c`, `62ad83f617d42bcb2778f678a5e4b690560bb4b3e3f1e0cc38130483f7fd0b30`, and `a9255bdb22014459596fc667a833ef0a5a283dad7b16922003bbe9e0b07328ca`; there is no Rust-path or `ralph.json` diff from `84e5765`.
- `cargo fmt --all -- --check` remains nonzero on the same broad pre-existing formatting drift across untouched Rust crates, including the intentionally frozen optional-null INT-008 suite. This pass does not rewrite those bytes.
- An additional replay of the Rust CI commands was attempted because the workflow itself changed. `cargo check --workspace --all-targets` progressed through the workspace but failed while writing server metadata with `No space left on device`; `cargo test --workspace --all-targets` likewise ran many suites before `catalog::stats::save_and_load` could not write a temp file for the same storage-full condition; `cargo clippy --workspace --all-targets -- -D warnings` could not create its target fingerprint. `df` confirms the repository filesystem is at 100% with zero available bytes while `/tmp` still has space. These are environment-capacity failures, not a changed-Rust regression; no cleanup/reset/delete operation is performed by this pass.

## Post-policy residual source re-evaluation

The immediate residual audit on clean `main` at `188d786` re-derived the live nonaccepted projection from Ralph, task/worklog state, git history, `FEATURES.md`, behavior-surface rules, the final DISC reconciliation/evidence catalogs, and locked upstream commits rather than assuming the protection-only commit could not affect ownership. The checked-in exhaustion artifact exactly equals `build_expected_ledger(...)`: all 86 live nonaccepted rows are represented once, their controller statuses/requirements/task artifacts/implementation receipts remain current, and every pinned evidence id resolves through the locked source catalogs. No Ralph, behavior-rule, evidence-catalog, reconciliation/manifest, requirement, or Rust product path changed from `84e5765`; only DISC policy/task/worklog documentation changed in the protection commit.

The strongest apparent residual candidates remain arithmetic singletons, not source-grounded one-to-one contracts. `ROUTE-009` is the only unresolved row left across several dashboard/account surfaces, but those reviewed families still independently cover persistence/schema, secret-free status projection, settings mutation/environment side effects, and routing semantics. `ROUTE-010` is the lone unresolved row on `9router.routing`, yet its Ralph story remains the generic DISC-002 placeholder and the reviewed family explicitly leaves handler fallback/stream-commit, combo routing, provider cooldown, and quota behavior unexhausted. `EXT-005` still spans broad extensibility plus legacy compatibility, and `WEB-004` still spans heterogeneous hosted enterprise remote behavior plus server control-plane concerns. Singleton set intersection therefore remains insufficient to assign a native Rust boundary or strict-TDD contract.

This pass did uncover a narrower evidence-accounting weakness: row-level aggregation can hide evidence classes missing on one constituent surface by borrowing the same class from another mapped surface. `sources/backlog-exhaustion.json` now records `surfaceEvidenceGaps` for every residual row, derived directly from each reviewed DISC surface, and `tools/validate_backlog_exhaustion.py` validates that projection. The unresolved product-relevant gap is now explicit: `INT-010`, `SHARE-003`, and `WEB-004` each map through `opencode.enterprise-remote`, whose reviewed partial evidence has pinned source/caller/test material but no dedicated `spec` evidence. That missing spec is preserved as a blocker/decomposition fact rather than filled with an unrelated spec from another surface.

The same projection preserves other non-promotional partial-evidence facts: stale-local `ROUTE-005`/`ROUTE-007` still traverse `9router.model-combos` without direct test evidence, while dependency-constrained app/desktop rows retain their missing client/desktop spec evidence. Those rows do not reopen accepted/local work or authorize UI/TUI/desktop implementation; they simply make the remaining reconciliation gap explicit and fail-closed. Bootstrap coverage weakens `opencode.enterprise-remote` further and proves the per-surface missing-class projection changes instead of being masked by another surface.

The exact locked OpenCode inventory was checked for a plausible enterprise specification before leaving that gap open. `packages/enterprise/README.md` exists at pinned commit `95daf90670b7c039c436c85537da5fbfe2205b41`, blob `9337430cfd31be33675b8e7336b9260906e98440`, but the reviewed 32-line file is only the stock SolidStart create/develop/build scaffold. It does not specify share synchronization, Durable Object/WebSocket lifetime, hosted auth/token exchange, support relay, installation lookup, resource bounds, or failure semantics. It is therefore deliberately **not** added as `spec` evidence merely to make `opencode.enterprise-remote` look complete; the missing spec remains explicit.

No materially new pinned evidence establishes a safe one-to-one native product contract. The nine explicit blockers and dependency-constrained UI/TUI remain closed on their recorded conditions, all 34 unresolved/decomposition rows remain unresolved, and DISC-003 remains **IN PROGRESS / NOT ACCEPTED** with `in-progress-not-release-evidence`.
