# INTEGRATION-11 — RED-validity matrix + guard-port/codex-bounds + wiring + flip requests (v11)
...
Supersedes INTEGRATION-10 where disk disagrees (marked CORR).
Inputs: `worklog/INTEGRATION-10.md`, `worklog/GUARD-TRIAGE-11.md`,
`worklog/TURN-STREAM-GATE.md`, `worklog/RED-VALIDITY-INT.md` (+ uA retry §),
`worklog/RED-VALIDITY-INT-FINAL.md` (NEW vs v10 — was ABSENT),
`worklog/RED-VALIDITY-OPS2.md`, `worklog/RED-VALIDITY-OPS-FINAL.md` (NEW vs v10 — was ABSENT),
...
(re-verified this lane via python count on `userStories[].status`; unchanged vs v8/v9/v10). NOT edited.

## 1. Stub / ignore scan (this lane)

- Strict grep: `grep -rn --include='*.rs' -e 'todo!' -e 'unimplemented!' -e '#\[ignore\]' crates/`
  → **0 hits**. Log: `/tmp/opencode/wP-stub.log` (0 lines). Classify: CLEAN.
- Loose `ignore` matches remain only known-benign (`eq_ignore_ascii_case`,
  doc "ignore leading/trailing whitespace" in `diff_tool.rs`, etc. per v8/v9/v10 §1).
  Zero real stubs. No action.

## 2. Fmt + status (this lane)

- `cargo fmt --check 2>&1 | grep -c 'Diff in'` → **48** (unchanged vs v10 = 48; CORR vs v6–v9 = 43).
- `git status --porcelain` (bare `git` authority):
  **204 M + 193 ?? = 397 (wc -l 396, one-line awk/wc rounding)**.
  CORR vs INTEGRATION-10 (204 M + 188 ?? = 392) = **0 tracked, +5 untracked**.
  CORR vs GUARD-TRIAGE-11 (204 M + 185 ?? = 389) = 0 tracked, +8 untracked drift.
...
| storage | `crates/storage/src/lib.rs` | UNPLANNED-uncommitted | +18/-9 fmt-only reflows (unchanged vs v10; zero mod add/remove) | none | none — attribute or revert |
| tools | `crates/tools/src/plugin_transform.rs` | GUARD-PORT-landed, uncommitted (unchanged vs v10) | +9/-2: unknown-scope guard `entries.iter().any` + invariant docs (§3a) | EXT-TWINS-DISPOSITION pair-1 | needs canonical 5/5 re-run post-port |

- 6 lib.rs files differ (same set as v8/v9/v10 §3). Storage hunk still +18/-9
...
- Status: **PORT STILL LANDED in worktree, uncommitted, UNCHANGED vs v10.**
...
- `git diff HEAD -- crates/providers/src/codex_oauth.rs
  crates/providers/tests/codex_oauth.rs` → **empty (clean, unchanged vs v10)**.
...
CORR vs v10: INT-FINAL and OPS-FINAL files NOW EXIST (were ABSENT at v10);
verdicts below cite FINAL files as primary receipt, uA/OPS2 as epoch detail.
...
2. Guard FAIL backlog exhaustion (owner: controller). 122 errors exit 1,
   byte-identical sets vs TRIAGE-10/11. 69 FEATURES sync + 53 ledger/gap; no lane edits ralph.json.
3. Dirty-tree RED caveat on INT + OPS + frozen-test reformats (owner: INT/OPS lanes + verifier).
   ...
   v10 "ABSENT" gap CLOSED), but logged against CURRENT dirty bytes, not
   pristine commit; frozen tests carry uncommitted rewraps. Neither slice
   accept-flippable until quiesced-tree re-run. Verifier must read FINAL files
   as the receipts, not wait on further files.
...
5. PROV-017 fix gate + fmt/untracked drift + AUTO-004 tokio gap (owner: providers lane +
   ...
   `cargo fmt --check` 48 (flat vs v10); status 392→397 (+5 untracked worklogs).
