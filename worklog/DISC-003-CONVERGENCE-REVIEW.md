...
## Session
ses_f32ffe76efferbs2dfHSfN6vFt
...
- `tools/convergence_gate.py` defines findings via `ledger_errors()`:
...
| FIX-TIMELINE | Map into AUD-013 or TUI-010 repair children | claims.json completedNote; worklog/FIX-TIMELINE.md | AUD-013 shard |
...
| PROV-021 | Remain in legacy set A3; not a plan story | ralf.json PROV-021 status=accepted; claims.json completed | AUD-002 shard |
...
| RUN-001 | Remain in legacy set A3; not a plan story | ralh.json RUN-001 status=accepted; claims.json completed | AUD-010 shard |
