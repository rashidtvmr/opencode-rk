# RED-VALIDITY-AUTO3 (sole-writer wave, rev 248f519)
...
first (2.4-2.8Gi avail, abort-line 1GiB never hit), --test-threads=1,
...
- Stub: `detach` owner guard `!=` -> `==` (python one-liner, one line).
- RED /tmp/opencode/xA-AUTO-004-red.log: compiles, 3 passed / 2 failed.
  FAIL auto_004_t02 (tests/delegation_lane.rs:37 owner detach wrongly
  denied: NotOwner), FAIL auto_004_t03 (:60 detach wrongly denied).
- GREEN /tmp/opencode/xA-AUTO-004-green.log: 5 passed / 0 failed.

## AUTO-006 (owned impl driver_lane.rs; frozen tests untouched)
- Stub: `LeaseTable::release` guard `==` -> `!=` (python one-liner).
- RED /tmp/opencode/xA-AUTO-006-red.log: compiles, 4 passed / 1 failed.
  FAIL auto_006_t02 (tests/driver_lane.rs:70 non-owner release wrongly
  succeeded: `()`).
- GREEN /tmp/opencode/xA-AUTO-006-green.log: 5 passed / 0 failed.

## AUTO-005 declarative (owned gated tests; impl owned delegation_lane.rs)
- Behavior stub: `broker_allows` Allow-only -> inverted (`!matches!`).
- RED /tmp/opencode/xA-AUTO-005-red.log: compiles, 0 passed / 5 failed
  (fa3_gated_t01 delegation_gated.rs:18 deny admitted Handle;
  t02 :34 human admitted Handle; t03 :44 allow denied BrokerDenied;
  t04 :55 pool Allow acquire false; t05 :77 pool Allow acquire false).
- GREEN /tmp/opencode/xA-AUTO-005-green.log: 5/5 pass.
- Live tool PASS/mutated evidence (rev auto005-rev-001, /tmp/opencode/xA005):
  PASS exit 0 all-pass (report-pass.json); edited 1-byte (v1->v2) exit 2
...
- /tmp/opencode/xA-agents-full-green.log: lib 15 + gated 5 + lane 5 +
  driver 5 + turn 5 = 35 passed, 0 failed. exit 0.
