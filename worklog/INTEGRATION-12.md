# INTEGRATION-12 — RED-validity matrix + guard-port/codex-bounds + wiring + flip requests (v12)
...
Supersedes INTEGRATION-11 where disk disagrees (marked CORR).
Inputs: `worklog/INTEGRATION-11.md`, `worklog/GUARD-TRIAGE-11.md` (latest on disk),
`worklog/TURN-STREAM-GATE.md`, `worklog/RED-VALIDITY-INT.md` (+ uA retry §),
`worklog/RED-VALIDITY-INT-FINAL.md`, `worklog/RED-VALIDITY-OPS2.md`,
...
`RED-VALIDITY-EXT1/EXT1B/EXT2/AUTO/TOOL/ACPSDK.md`,
...
Guard (GUARD-TRIAGE-11, no re-run this lane): validate_repository exit 1,
122 errors; validate_plan exit 1, 133 = 122 + 11 unknown prefixes
(ACP/HEAD/RUN/SDK/SYNC/WSX); `git diff --check` clean (exit 0).
`ralph.json`: 258 stories, 176 accepted / 44 in-progress / 38 not-started
(re-verified this lane via python count on `userStories[].status`; unchanged vs v8–v11). NOT edited.

## 1. Stub / ignore scan (this lane)

- Strict grep: `grep -rn --include='*.rs' -e 'todo!' -e 'unimplemented!' -e '#\[ignore\]' crates/`
  → **0 hits**. Log: `/tmp/opencode/xQ-stub.log` (0 lines). Classify: CLEAN.
- Loose `ignore` matches remain only known-benign (`eq_ignore_ascii_case`,
  doc "ignore leading/trailing whitespace" in `diff_tool.rs`, etc. per v8–v11 §1).
  Zero real stubs. No action.

## 2. Fmt + status (this lane)

- `cargo fmt --check 2>&1 | grep -c 'Diff in'` → **48** (unchanged vs v10/v11 = 48; CORR vs v6–v9 = 43).
- `git status --porcelain` (bare `git` authority):
  **204 M + 205 ?? = 409 (wc -l 408, one-line awk/wc rounding)**.
  CORR vs INTEGRATION-11 (204 M + 193 ?? = 397) = **0 tracked, +12 untracked**.
  CORR vs GUARD-TRIAGE-11 (204 M + 185 ?? = 389) = 0 tracked, +20 untracked drift.
...
| security | `crates/security/src/lib.rs` | UNPLANNED-uncommitted | 1-line alpha swap (unchanged, numstat 1/1) | none | none — attribute or revert |
| tools | `crates/tools/src/lib.rs` | UNPLANNED-uncommitted | 1-line alpha swap (unchanged, numstat 1/1) | none | none — attribute or revert |
| storage | `crates/storage/src/lib.rs` | UNPLANNED-uncommitted | +18/-9 fmt-only reflows (unchanged vs v10/v11; zero mod add/remove) | none | none — attribute or revert |
| tools | `crates/tools/src/plugin_transform.rs` | GUARD-PORT-landed, uncommitted (unchanged vs v10/v11) | +9/-2: unknown-scope guard `entries.iter().any` + invariant docs (§3a) | EXT-TWINS-DISPOSITION pair-1 | needs canonical 5/5 re-run post-port |

- 6 lib.rs files differ (same set as v8–v11 §3). Storage hunk still +18/-9
...
- No `crates/*/tests/zz_*` in tree (probe absent, unchanged).
...
- Status: **PORT STILL LANDED in worktree, uncommitted, UNCHANGED vs v10/v11.**
  `git diff HEAD -- crates/tools/src/plugin_transform.rs` = +9/-2:
...
- `git diff HEAD -- crates/providers/src/codex_oauth.rs
  crates/providers/tests/codex_oauth.rs` → **empty (clean, unchanged vs v10/v11)**.
  No bounds fix, no test addition landed. Frozen tests untouched.
...
INT-FINAL and OPS-FINAL files EXIST (PRIMARY receipts); uA/OPS2 as epoch detail.
...
| EXT-001/002/004/005/006/008 | behavior-stub per file (1–2 fail each: T03 boundary arms) + 5/5 GREEN, sha pre==post | `worklog/RED-VALIDITY-EXT1.md` + `RED-VALIDITY-EXT1B.md` + `/tmp/opencode/rB-*-red.log`, `/tmp/opencode/wB-*-red.log` | **RED-VALID (strong, behavior)** |
...
| OPS-001..009 | OPS2 behavior RED: signatures kept, one wrong behavior per file, suites COMPILE + fail on assertions (zero E0432/E0433) + 5/5 GREEN each, sha pre==post | `worklog/RED-VALIDITY-OPS-FINAL.md` (PRIMARY; verifies OPS2) + `worklog/RED-VALIDITY-OPS2.md` + `/tmp/opencode/uB-*-red.log` (9/9 present) | **RED-VALID (strong, behavior). Caveat retained: tree dirty on entry (frozen tests pre-modified by another lane), so wiring proven on CURRENT content, NOT pristine-commit validity. Re-do on quiesced tree before acceptance flip.** |
| INT-001/002/003/005/006/007/009/010 | uA retry ON CURRENT bytes: per-file temp behavior-stub → compile+fail (INT-001 2/3, INT-002 0/5, INT-003 3/2, INT-005 3/2, INT-006 4/1, INT-007 4/1, INT-009 2/3, INT-010 0/5) → byte-identical restore → 5/5 GREEN each | `worklog/RED-VALIDITY-INT-FINAL.md` (PRIMARY; 8-row matrix + sha256) + `worklog/RED-VALIDITY-INT.md` §Retry + `/tmp/opencode/uA-INT*-red.log`, `uA-INT-green1/2.log` | **RED-VALID (strong, behavior). Same dirt caveat as OPS: 6/8 src files carried fmt-only hunks at stub time; RED logged against current bytes, restore verified IDENTICAL. Frozen tests/lib.rs/ralph.json never touched.** |
...
| EXT-001/002/004/005/006/008 | not-started | review → in-progress? RED-VALID §4 | RED-VALIDITY-EXT1 + EXT1B |
...
| OPS-001..009 | ? | HOLD → behavior-RED proven (§4 FINAL) BUT dirty-tree caveat; re-run on quiesced tree before accept-flip | `worklog/RED-VALIDITY-OPS-FINAL.md`, this file §4 |
| INT-001..010 | ? | HOLD → behavior-RED proven (§4 FINAL) BUT dirty-tree caveat; re-run on quiesced tree before accept-flip | `worklog/RED-VALIDITY-INT-FINAL.md`, this file §4 |
| Unknown prefixes SYNC/RUN/ACP/WSX/SDK/HEAD | n/a | controller: allowlist-or-map + reconcile accounting | plan validator 11 extra (GUARD-TRIAGE-11 §1) |
...
2. Guard FAIL backlog exhaustion (owner: controller). 122 errors exit 1,
   byte-identical sets vs TRIAGE-11. 69 FEATURES sync + 53 ledger/gap; no lane edits ralph.json.
...
5. PROV-017 fix gate + fmt/untracked drift + AUTO-004 tokio gap (owner: providers lane +
   ...
   `cargo fmt --check` 48 (flat vs v10/v11); status 397→409 (+12 untracked worklogs, CORR).
