...
- TUI-011 is intentionally not integrated yet: its product script conflicts with
  current main's `--help` identity gate and the security review found three
  defects requiring a new frozen RED first.
...
1. TUI-011 security RED, implementation repair, conflict resolution preserving
   both staged `--version` and current-main `--help` identity contracts.
