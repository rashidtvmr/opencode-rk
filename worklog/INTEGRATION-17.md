# INTEGRATION-17 — per-slice GREEN matrix + c-wave receipts + wiring + flip table (v17)
...
Supersedes INTEGRATION-16 where disk disagrees (marked CORR).
Inputs: `worklog/INTEGRATION-16.md`, `worklog/GUARD-TRIAGE-18.md` (newest),
`worklog/REL-T01-CONFIRM.md` (cC, newest REL receipt), `worklog/TURN-STREAM-GATE.md`,
`worklog/TWINS-WATCH4.md` (newest twins survey), `worklog/WEB-013.md` / `worklog/WEB-015.md`
(full-suite addenda bC/bD), `worklog/PROV-016-FREEZE.md` (bE), `worklog/REL-LANEGATE.md` (bF),
`worklog/REL-004.md` (accepted, out of flip set), on-disk diffs, `ralph.json` (read-only).
No `WEB-013-LIFT`, `WEB-015-LIFT`, `REL-T01-CONFIRM`-beyond-cC, `CONFIRM-CI`, `CONFIRM-CK`,
or `TWINS-WATCH5` lanes exist on disk (`ls -t worklog` newest = GUARD-TRIAGE-18,
REL-T01-CONFIRM; find for lift/confirm/watch5 = zero hits). c-wave = b-wave receipts
re-verified + cC REL-T01 re-confirm + TRIAGE-18 guard.

Guard: TRIAGE-17 (122 repo / 133 plan FAIL, 456 = 204 M + 252 ??).
TRIAGE-18 (newer): same 122/133 FAIL, diff-check 0, **462 = 204 M + 258 ??** (porcelain).
This lane: **464 = 204 M + 260 ??** (porcelain; +2 untracked vs TRIAGE-18 =
GUARD-TRIAGE-18.md + REL-T01-CONFIRM.md themselves). Tracked M flat at 204.
`rtk git status --short` shows truncated 461-line sample — filter artifact, porcelain
authoritative (TRIAGE-18 §verdict notes rtk truncation). Forbidden paths untouched
(TRIAGE-18: ralph.json/FEATURES.md/controller/PLAN.md NO_CONTROLLER_TOUCH).
GUARD RED — no integration commit.
`ralph.json`: 258 stories, **176 accepted / 44 in-progress / 38 not-started**
(re-verified this lane via python count on `userStories` key). NOT edited.

## 1. Stub / ignore scan (this lane)

- Strict grep: `grep -rn --include='*.rs' -e 'todo!' -e 'unimplemented!' -e '#\[ignore\]' crates/`
  → **0 hits** (exit 1, empty). Log: `/tmp/opencode/cM-stub.log` (0 lines, this lane). CLEAN.
  Matches STUB-FMT-8 (`/tmp/opencode/bO-stub.log`, 0 bytes).

## 2. Fmt + status (this lane)

- `cargo fmt --check 2>&1 | grep -c 'Diff in'` → **58** (unchanged vs v16/STUB-FMT-8; no fmt
  run this lane — write-shaped op banned).
- `git status --porcelain=v1`: **204 M + 260 ?? = 464** (tracked flat at 204; untracked +2 vs
  TRIAGE-18 = the two newest worklogs; drift = worklogs + additive suites only).
- `git diff HEAD --stat -- crates/`: **203 files, +2071/-1232** (+1 insertion vs v16
  +2070/-1232; drift = ext_replay_lane fmt reflow, §3).
- `git diff --check` → exit 0, no whitespace errors (TRIAGE-18).
...
| foundation | `crates/foundation/src/lib.rs` | WIRED-uncommitted | +5/-2 (unchanged) | FOUNDATION-WIRING-FINAL | bH-ops per-suite 5/5 |
...
| tools | `crates/tools/src/lib.rs` | UNPLANNED-uncommitted, **CORR vs v16** | **2/1: `pub mod ext_manifest_lane` ADDED (:19)** + tool_sandbox/quota alpha swap | none (parallel-lane wire, owner unknown) | none — attribute + freeze-or-drop before commit |
| storage | `crates/storage/src/lib.rs` | UNPLANNED-uncommitted, fmt-only | **+18/-9 reflows** (zero `pub mod`/`pub use` add/remove per grep; was +N fmt-only in v16, grown by sibling fmt churn) | none | none — attribute or revert |
| tools | `crates/tools/src/plugin_transform.rs` | GUARD-PORT-landed, uncommitted (unchanged) | +9/-2: unknown-scope guard + invariant docs | EXT-TWINS-DISPOSITION pair-1 | WATCH4 §3 byte-identical both sides; canonical 5/5 re-run still mandated §6 |
| tools | `crates/tools/src/ext_replay_lane.rs` | twin fmt reflow (guard INTACT) | **+6/-4: `any()` reflow to single line** (`disabled.contains && entries.any(scope)` preserved) | none (sibling fmt churn) | guard semantics unchanged; pair-1 drop still gated on §6 re-run |
| providers | `crates/providers/src/claude_oauth.rs` | FIX-landed, uncommitted (PROV-017 gate CLEARED) | +1/-1: raw `{LOOPBACK_REDIRECT_URI}` (unchanged) | PROV-VERIFY4 | 11 suites 56/56 GREEN |
| providers | `crates/providers/src/codex_oauth.rs` | clean impl | `git diff` empty (unchanged) — bounds pins live in NEW test files, impl untouched | PROV-016 bounds/bounds2 + PROV-016-FREEZE | 5/5 + 6/6 + 6/6 = 17/17 (bE-prov16.log) |
...
- CORR vs v16 §3: `ext_manifest_lane` is now WIRED (`pub mod ext_manifest_lane` at
  `crates/tools/src/lib.rs:19`, parallel-lane edit, no owning worklog). v16 "deliberately
  unwired" no longer holds. Integrator call flips: either keep-wire (attribute + freeze
  bytes-lane as canonical-adjacent suite) or revert-wire (back to `#[path]`-only); until
  decided, EXT-005 stays y-caveat (§4a, count-neutral).
- TWINS-WATCH4: 0/13 drift vs WATCH3; pair-1 guard PORTED both sides, bodies byte-identical
  (`plugin_transform.rs:128-139` vs `ext_replay_lane.rs:132-143` modulo fmt reflow);
  droppable by integrator (NOT done here). `codex_oauth.rs` impl clean re-confirmed
  (`git diff --quiet` → clean).

### 3a. SHARE wiring verdict (b-wave close, unchanged)
...
| PROV-016 boundary | bounds + bounds2 + PROV-016-FREEZE (bE) | 17/17 (frozen T01–T05 5/5 + B06..B11 6/6 + C12..C17 6/6); mut-controls FAIL-as-expected (B10-flip 5p/1f, C15-flip 5p/1f) | uD B3/B3alt/B3exp stub classes KILLED ×2 (freeze waiver still needed, §4b) |
...
| REL-001..003 | bF lanegate + **cC REL-T01-CONFIRM** | T01 triple GREEN ×2 + `cmp` byte-identical determinism + T02/T03 fail-triple spot checks; hashes match (`ab9b350e…`, `79be6ef1…`, `3a462032…`); cC re-confirms T01×2 + cmp clean (`b6d34527…`, `3600e645…`, `238bc203…`) | lane_gate.py has NO REL coverage (8 PASS modules, 4 UNRUN tests) — validator-only, verifier re-run pending |

### 4a. EXT-005 two-lane status (must-flag, UPDATED c-wave wiring drift)
...
- Bytes lane `ext_manifest_lane.rs` (UNWIRED-test, **NOW WIRED in lib.rs:19 by parallel
  lane**): zF pass proves bytes pipeline PRESENT; bB-ext005 PRE hashes match zF bytes
  exactly (impl `e54e9647…`, test `92701b3f…`).
- Gap-closed: **y** — EXT1-VERIFY4 bytes-pipeline gap closed by ext_manifest_lane
  (EXT-005.md:61). New decision forced by drift: keep the `pub mod ext_manifest_lane`
  wire (attribute + freeze bytes-lane) or revert it (back to `#[path]`-only). Until
  test-owner/integrator decides, EXT-005 stays y-caveat in §7 (count-neutral vs v16).

### 4b. PROV-016 freeze status (bE, unchanged)

- PROV-016-FREEZE.md pins exact sha256: impl `e16ef548…` (tracked clean, re-confirmed
  `git diff --quiet` clean this lane), frozen `codex_oauth.rs` `aa7459f6…`, bounds
  `9d2c8d87…` (B06..B11), bounds2 `3595ed6c…` (C12..C17). GREEN 17/17 (`bE-prov16.log`).
...
### 4c. WEB-013/015 full-suite lifts (bC/bD — why partial STANDS, no partial→y lift)
...
- Lift EXPLAINED: reason-probes 3/3 → full boundary suites 5/5 + 5/5 with single-test
  mut-controls is an evidence upgrade (boundary pinning), NOT a card-contract lift. Card
  T01–T05 (executor/replay for 013; membership/memory for 015) still absent per both
  cards' NOT-ACCEPTED boundary (`WEB-013.md:38`, `WEB-015.md:15-18`; no executor, no
  membership/memory authority exists to test against). Hence **partial STANDS** (§7 rows
  51/53) — count-neutral vs v16. A partial→y flip requires a future lane implementing the
  card contract + frozen T01–T05 RED/GREEN; verifier decides.
...
- zA/zC/zD/zG/zH stubs applied to CURRENT (dirty) bytes, restored byte-identical
  ...
  hashes but do not lift the dirty-tree qualifier on the RED half. cC (REL-T01-CONFIRM)
  is validator-script re-confirm, not product RED — does not touch this caveat.
...
| `EXT2-VERIFY4.md` (yD) | EXT-009/010/011/012 | GREEN 20/20. RED bite via xD 0/5. Guard-port hash PASS. |
...
| `REL-LANEGATE.md` (bF) + `REL-T01-CONFIRM.md` (cC) | REL-001/002/003 | T01 triple GREEN ×2 + determinism `cmp` PASS + fail-triple spot checks; lane_gate.py has NO REL lanes (8 PASS modules, 4 UNRUN tests). Validator-only; verifier re-run pending. |
...
- REL-T01 re-confirm (cC, validator scripts, no product RED): T01×2 per validator + `cmp`
  determinism; `tools/ fixtures/ ralph.json` must stay clean (`git status` clean at cC time).
...
UNCHANGED vs v16 — c-wave receipts are evidence upgrades + drift flags, zero flips (§4c).

c-wave deltas vs v16 (ALL COUNT-NEUTRAL — 77 / 2 / 3 unchanged):
- WEB-013/015 (bC/bD): NO partial→y lift — full boundary suites pin flags/reasons/
  determinism, NOT card T01–T05 (executor+replay / membership+memory absent per cards).
  Partial STANDS with rationale §4c.
- PROV-016 (bE/FREEZE, re-confirmed impl-clean this lane): stays y-caveat — both additive
  suites untracked, freeze waiver pending.
- REL-001..003 (bF + cC): stay **n** — cC re-confirms T01×2 GREEN + `cmp` determinism at
  pinned validator hashes, but receipts remain validator-script runs, NOT product-behavior
  RED; lane_gate.py defines NO REL lanes; verifier re-run + controller accept pending.
  Rationale: flipping n→y requires product-behavior RED (temp-stub or equivalent) +
  verifier-owned GREEN on quiesced tree; neither exists for REL.
- EXT-005: stays y-caveat — NEW drift forces keep-or-revert call on the
  `ext_manifest_lane` lib.rs wire (§3 CORR, §4a).
- SHARE/INT/OPS: y / y-caveat rows stand (§4h quiesced-tree re-run still required).
- REL-004 (`turn_cost_counters`, 5/5 GREEN + regression 45 lib pass) is `accepted` in
  ralph.json — outside the 82-row flip set, no row here.
...
| 7 | EXT-005 | in-progress | y-caveat | BYTES-LANE y (xC 3/2 + zF 4/1 probe + bB PRE hash-match, gap-closed y) BUT canonical `plugin_manifest` struct-shape only — INTEGRATOR keep-or-revert call on NEW `pub mod ext_manifest_lane` wire (§3 CORR, §4a) |
...
| 31 | REL-001 | in-progress | n | validator-only, bF T01 GREEN + cC T01×2 re-confirm + determinism PASS, verifier re-run pending |
...
| 51 | WEB-013 | not-started | partial | entry-point RED-VALID (s3 0/5) + uC WEAK-RED + reason 3/3 + full boundary suite 5/5 (bC) + mut-control; card T01–T05 still absent — NO partial→y lift, rationale §4c; controller decides flip |
...
| 53 | WEB-015 | not-started | partial | entry-point RED-VALID (s3 0/5) + uC WEAK-RED + reason 3/3 + full boundary suite 5/5 (bD) + mut-control; card T01–T05 still absent — NO partial→y lift, rationale §4c; controller decides flip |
...
EXT-005 card-vs-canonical gap → keep-or-revert the NEW `ext_manifest_lane` wire (§3 CORR, §4a).

## 8. Ranked blockers top 5 (owner lanes)

1. Guard/controller backlog exhaustion (owner: controller). 122 repo + 133 plan errors,
   exit 1, byte-identical classes vs TRIAGE-16/17/18 (one line-order swap only).
   Porcelain 204 M + 260 ?? = 464; diff-check clean. No lane edits ralph.json.
   Blocks every accept-flip; integration commit forbidden until guard GREEN.
2. Wired-but-uncommitted + ungated + unattributed lib.rs drift + LANDED guard-port
   un-gated (owner: wiring lanes + verifier + integrator).
   Server +20, sessions +3, foundation +5/-2 in worktree, zero committed.
   NEW drift this lane: `pub mod ext_manifest_lane` wire in `tools/lib.rs:19` (parallel
   lane, no owning worklog — keep-or-revert call); `ext_replay_lane.rs` +6/-4 fmt reflow
   (guard intact); storage +18/-9 fmt-only reflows (zero mod add/remove).
   security/tools lib.rs 1/1 swaps still no owning lane — attribute or revert before
...
3. AUTO-004/006 tokio gap (owner: controller + integration lane). PROPOSAL2 spec'd,
   NOT applied, controller dep approval either way. y-caveat rows 1/3 stay until card
   amendment (sync state machine) or tokio integration lane lands. Broker mirror
   SUFFICES for behavior evidence; mechanism contract does not.
4. Fmt/untracked drift (owner: integrator + test owners). `cargo fmt --check` steady at
   58 (no new drift this lane); status 461→464 (+3 untracked: GUARD-TRIAGE-18,
   REL-T01-CONFIRM, +1 lane file; tracked M flat at 204). Six additive suites (bounds,
   bounds2, 2×reason, 2×full) still untracked — freeze-or-drop decision before commit.
   Storage fmt reflow growth (+18/-9) is sibling churn, logic-untouched.
5. REL verifier-owned re-run (owner: verifier + controller). REL-001..003 stay n:
   bF T01 triple + cC T01×2 re-confirm + `cmp` determinism at pinned hashes
   (`ab9b350e…`, `79be6ef1…`, `3a462032…`) are validator-script receipts, NOT
   product-behavior RED; lane_gate.py defines NO REL lanes (8 PASS modules, 4 UNRUN).
   Flip n→y needs product-behavior RED + verifier-owned GREEN on quiesced tree.
   REL-004 accepted, out of scope. WEB-013/015 full suites stay partial (§4c).

Merge order: (i) attribute/revert §3 unattributed lib.rs drift incl. NEW
ext_manifest_lane wire keep-or-revert; (ii) verifier-logged serial `plugin_transform`
5/5 on ported bytes → drop `ext_replay_lane` same commit; (iii) freeze-or-drop the 6
additive suites (bounds, bounds2, 2×reason, 2×full); (iv) EXT-005 keep-or-revert (§4a);
(v) quiesce INT/OPS/SHARE owned paths + frozen tests → INT/OPS/SHARE + zB-cargo re-run
(§4h); (vi) lane gates GREEN per wired suite (serial mandates §6, incl. PROV-016 triple
+ REL-T01 cC commands); (vii) refresh stale status files; (viii) ONE integration commit
(tracked fixes + lib.rs wirings incl. PROV-017 one-liner, fmt at commit); (ix) untracked
modules only with passing gates; (x) controller syncs FEATURES.md/accounting + flips
per §7.
